...
@Component
public class ICTPBox1 implements BotHandler {

    private final PlayerRepository playerRepository;
    private final RequestRepository requestRepository;
    private final TelegramBot bot;

    public ICTPBox1(PlayerRepository playerRepository, RequestRepository requestRepository, TelegramBot bot) {
    ...
    public boolean CanHandle(Update update){
        ...
        boolean status = player.getStatus().equals(UserStatus.ICTP_BOX1);
    ...
    public void Handle(Update update) {
        if (update.message().text().contains("[Назад]")){
            ...
            request.setBox1(update.message().text());
    ...
    public static SendMessage OutputICTPBox1(Long chatId){
        ReplyKeyboardMarkup keyboard = new ReplyKeyboardMarkup("◀ [Назад]");
        keyboard.resizeKeyboard(true);
        return new SendMessage(chatId, "<b>[Введите никнейм нарушителя]</b>")
