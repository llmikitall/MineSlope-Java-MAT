...
@Component
public class ICTPBox1Manager {
    private final TelegramBot bot;
    private final PlayerRepository playerRepository;
    private final RequestDraftRepository requestDraftRepository;

    public ICTPBox1Manager(TelegramBot bot, PlayerRepository playerRepository, RequestDraftRepository requestDraftRepository) {
        this.bot = bot;
        this.playerRepository = playerRepository;
        this.requestDraftRepository = requestDraftRepository;
    }

    public void showICTPBox1(Player player){
        player.setStatus(UserStatus.ICTP_BOX1);
        playerRepository.save(player);

        ReplyKeyboardMarkup keyboard = new ReplyKeyboardMarkup("◀ [Назад]");
        keyboard.resizeKeyboard(true);
        bot.execute(new SendMessage(player.getTgId(),"<b>[Введите никнейм нарушителя]</b>")
                .parseMode(ParseMode.HTML)
                .replyMarkup(keyboard));
    }

    public void saveInput(Player player, String text){
        player.getCurrentRequest().setBox1(text);
        playerRepository.save(player);
